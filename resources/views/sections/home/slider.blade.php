        ...
        <div class="relative mx-auto max-w-7xl px-4 sm:static sm:px-6 lg:px-8">
            <div class="sm:max-w-lg">
                <h1 class="text-4xl font-bold tracking-tight text-gray-900 sm:text-6xl">
                    {{$page->meta($section->id)['title'][app()->getLocale()] ?? "Summer styles are finally here"}}
                </h1>
                <p class="mt-4 text-xl text-gray-500">
                    {{$page->meta($section->id)['description'][app()->getLocale()] ?? "This year, our new summer collection will shelter you from the harsh elements of a world that doesn't care if you live or die."}}
                </p>
            </div>
            <div>
                <div class="mt-10">
                    ...
                    <x-splade-link :href="$page->meta($section->id)['url'] ?? '#'" class="inline-block rounded-md border border-transparent bg-indigo-600 px-8 py-3 text-center font-medium text-white hover:bg-indigo-700">
                        {{$page->meta($section->id)['button'][app()->getLocale()] ?? "Shop Collection"}}
                    </x-splade-link>
                </div>
            </div>
        </div>
